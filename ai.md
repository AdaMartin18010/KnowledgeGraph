# ai prompt

```text
1.   分析2025年 所有国际wiki的对应的 与 知识图谱 相关的 
    和所有国际著名大学所有课程内容 
    和所有国际最新最成熟的技术规范和模型
    梳理各个主题的相关内容知识 分析论证的思路   
    保持中英文双语来解释核心概念定义和论证思路 
    保持哲科严谨风格和批判性审查

2.   保持哲科风格的批判性的分析 所有内容的相关性 知识 梳理分类 重构到 /docs 目录下
建立各个梳理后的主题 目录  

3.   重构并持续输出到 /docs 各个主题子目录下 完成内容的梳理和
规整 避免重复 和 规范所有内容的 形式化 多表征的内容 包含详细的论证过程 形式化证明过程 

4.  输出符合 数学规范的 形式化规范的 markdown 文件 --包含严格序号的目录 和  多种表征方式 比如 图 表 数学形式证明符号 等等 如果需要编程代码请尽可能使用 rust haskell lean等编程语言

 请严格按照序号的树形目录 如果有文档不规范请修正过来--(/docs 目录里面的所有文件)
-- 包含严格序号的目录 和  多种表征方式 比如 图 表 数学形式证明符号 等等
    内容规范 不重复 分类严谨 保持与当前最新最成熟的哲科工程想法一致 
    按照 /docs 里面的所有分析归纳 去重 重构 合并 等 后 的精炼 
    主题进行分类和创建子文件 生成对应的相关内容分析综合 归纳 合并 等 这一一系列的过程
    总之 /docs 里面的文件夹 文件 文件内容可以本地相互引用和链接跳转

5.  构建能持续性 不间断的上下文提醒体系 可以中断后再继续的进程上下文文档 主要由你自己决定
6.   保证所有的都符合规范和学术要求 内容一致性  证明一致性 相关性一致性 
go on  激情澎湃的 <(￣︶￣)↗[GO!]    如何做 请您自己规划保持上下文   并批量执行 因为速度太慢了 

处理原则
所有原有批判性分析、表格、流程图、工程案例等内容必须完整保留。
结构优化和编号仅是为了提升可导航性和一致性，不应以牺牲内容为代价。
如有遗漏，后续批次会补全并恢复所有原有批判性内容，并在规范化区块中说明修正。

接着上次未完成的子目录工作
```
